<div class="p-6 bg-white dark:bg-gray-900 dark:text-gray-100 rounded-xl shadow-lg border border-gray-200 dark:border-gray-800">
    {{-- Header --}}
    <div class="flex flex-wrap items-center justify-between mb-6">
        <h2 class="text-3xl font-extrabold tracking-tight">📋 All Questions</h2>
    ...
    <div class="flex flex-wrap gap-3 mb-6">
        <input type="text"
               ...
               class="px-4 py-2 border border-gray-300 dark:border-gray-700 rounded-lg w-full sm:w-1/3 focus:outline-none focus:ring-2 focus:ring-indigo-400 dark:bg-gray-800 dark:text-white transition"/>

        <select wire:model.live="subject"
                class="px-4 py-2 border border-gray-300 dark:border-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-400 dark:bg-gray-800 dark:text-white transition">
            <option value="">All Subjects</option>
            @foreach(\App\Models\Qbank\Subject::all() as $subj)
                <option value="{{ $subj->id }}">{{ $subj->name }}</option>
            @endforeach
        </select>

        <select wire:model.live="chapter"
                class="px-4 py-2 border border-gray-300 dark:border-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-400 dark:bg-gray-800 dark:text-white transition">
            <option value="">All Chapters</option>
            @foreach(\App\Models\Qbank\Chapter::all() as $chap)
    ...
    <div class="overflow-x-auto rounded-lg shadow border border-gray-200 dark:border-gray-800">
        <table class="min-w-full text-sm">
            ...
            @empty
                <tr>
                    <td colspan="6" class="text-center py-6 text-gray-500 dark:text-gray-400">No questions found.</td>
                </tr>
            @endforelse
            </tbody>
        </table>
